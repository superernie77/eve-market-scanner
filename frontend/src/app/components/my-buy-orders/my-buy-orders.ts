...
@Component({
  ...
  template: `
    <div class="header-bar">
      <div class="title-area">
        ...
        <span>My Active Buy Orders</span>
    ...
    <div class="empty-state" *ngIf="!isLoggedIn">
      ...
      <p>Login with your EVE Online account to see your active buy orders.</p>
    </div>
    ...
    <div class="empty-state" *ngIf="isLoggedIn && !loading && orders.length === 0">
      ...
      <p>No active buy orders found for this character or corporation.</p>
    </div>
    ...
    <div class="table-container" *ngIf="isLoggedIn && !loading && orders.length > 0">
      ...
      <div class="summary-bar">
        ...
        <div class="summary-item">
          ...
          <span class="summary-value isk">{{ totalBidValue() | number:'1.0-0' }} ISK</span>
        </div>
      </div>
      ...
      <table mat-table [dataSource]="orders" class="orders-table">
        ...
        <ng-container matColumnDef="price">
          <th mat-header-cell *matHeaderCellDef>Bid Price (ISK)</th>
          <td mat-cell *matCellDef="let row" class="num bid-price">{{ row.price | number:'1.2-2' }}</td>
        </ng-container>

        <ng-container matColumnDef="range">
          <th mat-header-cell *matHeaderCellDef>Range</th>
          <td mat-cell *matCellDef="let row">
            <span class="range-badge">{{ formatRange(row.range) }}</span>
          </td>
        </ng-container>
        ...
        <ng-container matColumnDef="totalBid">
  ...
  styles: [`
    ...
    .summary-value.isk { color: #ffb74d; }
    ...
    .bid-price { color: #ffb74d; font-weight: 600; }
...
export class MyBuyOrdersComponent implements OnInit {
  private marketService = inject(MarketService);
  private authService   = inject(AuthService);
  private cdr           = inject(ChangeDetectorRef);

  displayedColumns = ['source', 'typeName', 'regionName', 'price', 'range', 'volume', 'totalBid', 'issued', 'expires'];
  ...
  load() {
    this.loading = true;
    this.marketService.getMyBuyOrders().subscribe({
      next: (data) => {
        this.orders = data;
        this.loading = false;
  ...
  totalBidValue(): number {
    return this.orders.reduce((sum, o) => sum + o.price * o.volumeRemain, 0);
  }
  ...
  formatRange(range: string | null): string {
    if (!range) return '—';
